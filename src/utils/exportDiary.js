...
// 'YYYY-MM-DD' → '2.7.2026' (rozpis zaplatených záloh)
function skDate(iso) {
  if (!iso) return ''
...
export async function exportAllBookings() {
  ...
  for (const b of data ?? []) {
    ...
    const row = ws.addRow({
      ...
      // prázdne (NULL) polia ostávajú prázdne aj v exporte
      expectedGuests: b.expected_guests ?? '',
      estimatedPrice: b.estimated_price != null ? Number(b.estimated_price) : '',
      // decoration = všeobecné poznámky z formulára rezervácie;
      ...
      depositPayments: payments
        .map(p => `${skDate(p.date)} – ${Number(p.amount)} €`)
        .join(' • '),
